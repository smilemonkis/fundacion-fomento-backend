...
public class ParchateService {
    ...
    public ParchateResponse obtenerPorId(Integer id) {
        return parchateRepository.findById(id)
                .map(this::toDto)
                .orElseThrow(() -> new ResourceNotFoundException("Parchate no encontrado"));
    }
    ...
    public Page<ParchateResponse> listar(String tipo, String ubicacion, Boolean activo, Pageable pageable) {
        ...
        if (tipo != null && !tipo.isBlank()) {
            page = parchateRepository.findByTipoIgnoreCaseAndActivo(tipo, soloActivos, pageable);
        } else if (ubicacion != null && !ubicacion.isBlank()) {
            page = parchateRepository.findByUbicacionIgnoreCaseAndActivo(ubicacion, soloActivos, pageable);
        } else if (activo != null) {
    ...
    private ParchateResponse toDto(Parchate p) {
        ...
        return new ParchateResponse(
                ...
                galeria, p.getTipo(), p.getUbicacion(), p.getDireccion(), p.getUrlMapa(),
                p.getFechaEvento() != null ? p.getFechaEvento().toString() : null,
                p.getActivo(), p.getCreatedAt().toString(), p.getUpdatedAt().toString()
        );
    }
}